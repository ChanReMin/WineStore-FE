...
export default function ProfileHeader({
  ...
  const handleAvatarSave = async (file: File, previewUrl: string) => {
    try {
      // Call API to upload avatar using /api/v1/users/avatar
      const response = await profileService.uploadAvatar(file);

      // Update local state with new avatar URL from server
      const newAvatarUrl = response.avatar || previewUrl;
      setCurrentAvatar(newAvatarUrl);
      onAvatarUpdate?.(newAvatarUrl);

      toast.success(t("avatarUpdateSuccess") || "Avatar updated successfully");
    } catch (error) {
      toast.error(t("avatarUpdateError") || "Failed to update avatar");
